...
func TestPrepareMountArgs(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "should not enable cloud profiler when enable-cloud-profiler=false is explicitly passed",
			mc: &MountConfig{
				BucketName: "test-bucket",
				BufferDir:  "test-buffer-dir",
				CacheDir:   "test-cache-dir",
				ConfigFile: "test-config-file",
				TempDir:    "test-temp-dir",
				Options:    []string{util.EnableCloudProfilerForSidecarConst + "=true", util.PodNameConst + "=my-pod", util.PodUIDConst + "=my-uid", util.EnableCloudProfilerConst + "=false"},
			},
			expectedArgs: map[string]string{
				"app-name":                    GCSFuseAppName,
				"temp-dir":                    "test-buffer-dir/temp-dir",
				...
				"uid":                         "0",
				"gid":                         "0",
				util.EnableCloudProfilerConst: "false",
			},
			expectedConfigMapArgs: map[string]string{
				"logging:file-path": "/dev/fd/1",
				"logging:format":    "json",
...
func TestMountErrorFileCleanup(t *testing.T) {
	...
	mc := &MountConfig{
		...
		ErrWriter:  NewErrorWriter(filepath.Join(tempDir, "error-log")),
	}
